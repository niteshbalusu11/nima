...
Migration 6 tags newly extracted groups with the fixed YuNet/SFace pipeline version. Earlier unversioned groups stay anonymous and cannot be enrolled. Capture comparison runs on dashboard reads without another queue or object-store copy. Named matching remains **disabled by default and on production**. For the isolated consented local trial, `FACE_RESEARCH_LOCAL_TRIAL=1` with `APP_ENV=development` enables a fixed cosine threshold of 0.30 and ambiguity margin of 0.05 for the pinned model version. The first uploaded video supplied four calibration face groups; a separate video supplied three evaluation groups. The trial produced one clear photo-reference match in each video, one ambiguous group, and four unnamed groups. This small set supports a local demonstration only; it does not establish accuracy for deployment. See [the research plan](../docs/consented-face-recognition-plan.md) for consent, evaluation, withdrawal, and demo gates. Do not add `FACE_RESEARCH_ACCOUNT_ID` or the local trial switch to Fly until a larger held-out evaluation passes the release gate.
