...
### Consent-based face research

The cross-capture study is off by default. Once enabled, a super-admin can opt in a capture from any active invited account after explicit participant-consent confirmation. Only opted-in captures participate; an enrolled reference can match an opted-in capture owned by a different account. Migration 10 makes older capture opt-ins and named enrollments ineligible for this wider scope. The operator must confirm capture consent again, then remove and re-enroll an older reference after confirming named enrollment consent again. No account ID or embedding is accepted from the browser. At most 20 reference people are stored across the study. Removing an enrollment or opting out a capture stops named comparisons on the next read; capture deletion also removes its research records. Anonymous face crops remain until the capture owner deletes the capture. SQLite backups retain prior snapshots according to the backup policy.

Migration 9 tags newly extracted groups with the fixed YuNet/SFace pipeline version. Earlier unversioned groups stay anonymous and cannot be enrolled. Capture comparison runs on dashboard reads without another queue or object-store copy. Named matching is disabled by default. For the isolated consented local trial, `FACE_RESEARCH_LOCAL_TRIAL=1` with `APP_ENV=development` enables a fixed cosine threshold of 0.30 and ambiguity margin of 0.05 for the pinned model version. A production research pilot requires `APP_ENV=production` and `FACE_RESEARCH_PRODUCTION_PILOT=1`; it does not require a separate research account. Setting `APP_ENV=development` on Fly does not enable production matching and relaxes HTTPS validation. Every capture still requires explicit opt-in, and every named reference requires separate enrollment. The first uploaded video supplied four calibration face groups; a separate video supplied three evaluation groups. The trial produced one clear photo-reference match in each video, one ambiguous group, and four unnamed groups. This small set supports a limited demonstration only; it does not establish general accuracy. See [the research plan](../docs/consented-face-recognition-plan.md) for consent, evaluation, withdrawal, and demo gates.
