...
4. **End-to-end and deployment.** Verify concurrent redemption creates only one account, session persistence still works, and the live video/photo tests still pass. Check both iOS builds and the admin-to-member QR flow on two phones. Run numbered SQLite migrations from `server/migrations.go` on app startup before serving requests. Roll back and stop on failure; never reset the database automatically. Migration 1 adopts the current prerelease schema. Append future changes as migrations. Push tested server changes to `master` for CI deployment, then create the first admin invite through the Fly CLI.
