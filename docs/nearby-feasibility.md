...
- Implement production on-device key generation and enrollment in the identity milestone. Imported development fixtures must not become a shipping dependency.

The next implementation slice is device identities, session binding, and directional peer approvals. Signed media records, durable nearby copies, delegated uploads, multiple recipients, and live viewing follow the dependencies in the engineering plan.
