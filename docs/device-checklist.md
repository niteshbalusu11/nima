...
1. Install Witness through ProData internal TestFlight (see [release instructions](testflight.md)). Team `GNQ3HY2357`, bundle ID `com.prodata.uploadvideo`, and the deployed HTTPS API are configured. Confirm this build's physical-device behavior before pilot use.
...
7. Confirm low-storage stops capture visibly and preserves pending files. The pilot retains saved files as well, with a 3 GiB cap.
