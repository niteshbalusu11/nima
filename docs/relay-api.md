...
Delegated uploads require `NEARBY_RELAY_ENABLED=true`. The default is disabled; `server/fly.toml` enables them on Fly. When disabled, recipients retain their verified local copies and show “cloud backup unavailable.” The foreground worker retries automatically after the server enables relay uploads.
